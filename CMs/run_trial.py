import numpy as np
from relation3 import relation_map,two_ship,three_ship,solve_r2,solve_r3
from acc_n_1 import cal_acc
from acc_al_2 import cal_al
...
for L0 in [4]:
    for L1 in [2]:
        ...
        tr1=np.zeros(30)
        d_old=np.zeros((9,30))
        d_old1=np.zeros((9,30))
        acc_t=0
        for ii in range (0,30000):
            s=0
            s1=1
            d=np.load('D:\\zsk\\RAVEN\\data_9\\'+str(ii)+'.npy')
            ...
            relationship_two_30[:,:,np.where((ii-tr)>500)[0]]=0
            relationship_three_30[:,:,:,np.where((ii-tr1)>500)[0]]=0
            d_old[:,np.where((ii-tr)>500)[0]]=0
            d_old1[:,np.where((ii-tr1)>500)[0]]=0
            count[np.where((ii-tr)>500)[0]]=0
            count1[np.where((ii-tr1)>500)[0]]=0
            if ii%1000==1:
#                 print(count)
#                 print('count1')
#                 print(count1)
#                 np.save('CM5\\ts2_final'+str(ii)+'_'+str(L0)+'_'+str(L1)+'.npy',relationship_two_30)
#                 np.save('CM5\\ts3_final'+str(ii)+'_'+str(L0)+'_'+str(L1)+'.npy',relationship_three_30)
                o,p=cal_al(relationship_two_30,relationship_three_30,L0,L1,ii)
                acc_t=cal_acc(relationship_two_30,relationship_three_30,L0,L1,o,p)
                print(str(ii)+','+str(acc_t))
            if acc_t>0.997:
            ...
            if ii==29999:
                relationship_two_30[:,:,np.where(count<2)[0]]=0
                relationship_three_30[:,:,:,np.where(count1<2)[0]]=0
        np.save('CM6\\ts2_final'+'_'+str(L0)+'_'+str(L1)+'.npy',relationship_two_30)
        np.save('CM6\\ts3_final'+'_'+str(L0)+'_'+str(L1)+'.npy',relationship_three_30)
        o,p=cal_al(relationship_two_30,relationship_three_30,L0,L1,ii)
        #print(o)
        #print(p)
        np.save('CM6\\o_final'+str(L0)+'_'+str(L1)+'.npy',o)
        np.save('CM6\\p_final'+str(L0)+'_'+str(L1)+'.npy',p)
#         acc_all[L0-4,L1-1]=cal_acc(relationship_two_30,relationship_three_30,L0,L1,o,p)
#         print(acc_all[L0-4,L1-1])
# print(acc_all)
np.save('CM5\\acc_final.npy',acc_all)
